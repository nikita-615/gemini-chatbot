...
def brand_df_multi(df):
    ...
    if selected_brand != 'All':
        temp_df = brand_df[brand_df['Brand'].isin(selected_brand)]
        return temp_df
    else:
        return brand_df
...
def state_df_multi(df):
    ...
    if selected_state != 'All':
        temp_df = state_df[state_df['State'].isin(selected_state)]
        return temp_df
    else:
        return state_df
...
def filtered_df(df):
    col1, col2 = st.columns(2)
    with col1:
        brand_df = brand_df_multi(df)
    with col2:
        state_df = state_df_multi(brand_df) if len(brand_df) > 0 else state_df_multi(df)
